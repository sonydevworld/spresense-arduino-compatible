/*
 *  rtc_timetone.ino - Example for RTC time tone
 ...
 *
 *  This is a example for RTC time tone.
 *  This example generate the beep of time tone every 10 seconds.
...
{
  // Generate a beep during the specified duration
...
void toneClock()
{
  ...
  // Tone when the second is updated
  if (now != old) {
    int tmp = now.second() % 10;
...
void setup()
{
  ...
  Serial.println("Example for time tone");

  // Initialize Audio beep library
...
void loop()
{
  ...
  // Generate the beep of time tone
  toneClock();
}
